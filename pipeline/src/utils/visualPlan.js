...
const inferSectionType = ({ title = "", body = "", index = 0, totalSections = 1 }) => {
  ...
  if (/intro|abertura|hook|contexto/.test(normalized)) {
    return "intro";
  }

  if (/fechamento|conclusao|cta|encerramento/.test(normalized)) {
    return "outro";
  }

  if (index === 0 && /(tens de conhecer|explorar|tres cidades|algo especial)/.test(normalized)) {
    return "intro";
  }

  if (
    index === totalSections - 1 &&
    (wordCount(body) <= 26 || /(melhor que portugal tem para oferecer|experiencias completamente diferentes)/.test(normalized))
  ) {
    return "outro";
  }
...
const buildSectionTitle = ({ body = "", topic = "", sectionType = "body", index = 0 }) => {
  ...
  if (sectionType === "intro") {
    return `Abertura em ${topicAnchor}`;
  }

  if (sectionType === "outro") {
    return `Fechamento em ${topicAnchor}`;
  }
...
const buildChunkTitle = ({ sectionTitle = "", chunk = "", chunkIndex = 0, totalChunks = 1, sectionType = "body" }) => {
  ...
  if (theme && !["abertura", "encerramento"].includes(theme)) {
    return `${sectionTitle}: ${theme}`;
  }

  if (totalChunks > 1 && sectionType === "body") {
    return `${sectionTitle} ${chunkIndex + 1}`;
  }
...
const guessSectionKeywords = (title = "") => {
  ...
  if (/intro|abertura|hook|contexto/.test(normalized)) {
    return ["aerial view", "travel destination", "city skyline", "landscape"];
  }

  if (/custo|orcamento|budget|logistica|transporte|planejamento/.test(normalized)) {
    return ["budget travel", "public transport", "street market", "city street"];
  }

  if (/ponto|lugar|ranking|imperdiveis|atracoes|atra[cç][aã]o/.test(normalized)) {
    return ["tourist attraction", "landmark", "historic district", "cityscape"];
  }

  if (/fechamento|conclusao|cta|encerramento/.test(normalized)) {
    return ["sunset skyline", "packing checklist", "travel planning", "city lights"];
  }

  if (/praia|ilhas|litoral|algarve/.test(normalized)) {
    return ["beach aerial", "coastline", "ocean cliffs", "travel lifestyle"];
  }

  if (/porto|ribeira|douro|ponte/.test(normalized)) {
    return ["river view", "waterfront", "bridge aerial", "historic district"];
  }

  if (/lisboa|lisbon|miradouro|bairro/.test(normalized)) {
    return ["city skyline", "historic district", "street life", "scenic viewpoint"];
  }
...
const buildSceneKeywords = ({ title, narrationExcerpt, topic, shots = [], entities = [], sectionType = "body" }) => {
  ...
  expandEntityKeywords([
    ...entities,
    ...extractEntityPhrases(`${title} ${narrationExcerpt}`, 4),
    ...extractEntityPhrases(topic, 2),
  ]).forEach((keyword) => {
  ...
  phraseTranslations.forEach(([pattern, keyword]) => {
    if (pattern.test(normalizedPool) && !keywords.includes(keyword)) {
      keywords.push(keyword);
    }
  });
...
const splitScriptIntoSections = ({ topic = "", scriptText = "", outlineSections = [] }) => {
  ...
  lines.forEach((line) => {
    ...
    if (headingMatch) {
      if (current && current.body.length) {
        sections.push({ ...current, body: current.body.join(" ").trim() });
      }
  ...
  if (paragraphs.length > 1) {
    if (outlineSections.length === paragraphs.length) {
      return paragraphs.map((body, index) =>
        createSection({
          title: outlineSections[index]?.title || "",
          body,
          topic,
          index,
  ...
  if (outlineSections.length) {
    ...
    return outlineSections
      .map((section, index) =>
        createSection({
          title: section?.title || `Cena ${index + 1}`,
          body: sentences.slice(index * chunkSize, (index + 1) * chunkSize).join(" ").trim(),
          topic,
          index,
...
const buildVisualPlan = ({
  ...
  const estimatedDurationSeconds = Math.max(
    30,
    Number(durationSeconds || 0) || Math.round(wordCount(scriptText) / 2.4)
  );
  ...
  sections.forEach((section) => {
    ...
    chunks.forEach((chunk, chunkIndex) => {
      ...
      scenes.push({
        ...
        keywords: buildSceneKeywords({
          title,
          narrationExcerpt: chunk,
          topic,
          shots,
