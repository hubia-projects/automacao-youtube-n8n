...
const config = {
  ...
  CONTRACT_COVERAGE_TOLERANCE: Number(process.env.CONTRACT_COVERAGE_TOLERANCE || 0),
  OUTPUT_ROOT,
  TEST_REPORTS_ROOT: process.env.TEST_REPORTS_ROOT || path.join("pipeline", "test_reports"),
  ...
  SHORT_VIDEO_MODE: process.env.SHORT_VIDEO_MODE === "true" || String(process.env.SHORT_VIDEO_MODE || "").toLowerCase() === "true",
  TARGET_VIDEO_DURATION_SECONDS: Number(process.env.TARGET_VIDEO_DURATION_SECONDS || 180),
  TARGET_SCRIPT_WORDS_MIN: Number(process.env.TARGET_SCRIPT_WORDS_MIN || 380),
  TARGET_SCRIPT_WORDS_MAX: Number(process.env.TARGET_SCRIPT_WORDS_MAX || 520),
  MIN_VIDEO_DURATION_SECONDS: Number(process.env.MIN_VIDEO_DURATION_SECONDS || 480),
  MAX_VIDEO_DURATION_SECONDS: Number(process.env.MAX_VIDEO_DURATION_SECONDS || 210),
  REQUIRE_REAL_AUDIO_TIMESTAMPS: process.env.REQUIRE_REAL_AUDIO_TIMESTAMPS === "true" || String(process.env.REQUIRE_REAL_AUDIO_TIMESTAMPS || "").toLowerCase() === "true",
  REQUIRE_REAL_CAPTIONS: process.env.REQUIRE_REAL_CAPTIONS === "true" || String(process.env.REQUIRE_REAL_CAPTIONS || "").toLowerCase() === "true",
  // C1+C2 — Local clip library quality gates and strict intent modes
  LOCAL_LIBRARY_MIN_SCORE: Number(process.env.LOCAL_LIBRARY_MIN_SCORE || 0.3),
  LOCAL_LIBRARY_STRICT_MIN_SCORE: Number(process.env.LOCAL_LIBRARY_STRICT_MIN_SCORE || 0.5),
  LOCAL_FOOD_STRICT_MODE: process.env.LOCAL_FOOD_STRICT_MODE === "true" || String(process.env.LOCAL_FOOD_STRICT_MODE || "").toLowerCase() === "true",
  LOCAL_CITY_STRICT_MODE: process.env.LOCAL_CITY_STRICT_MODE === "true" || String(process.env.LOCAL_CITY_STRICT_MODE || "").toLowerCase() === "true",
  LOCAL_LIBRARY_STRICT_RISK_THRESHOLD: Number(process.env.LOCAL_LIBRARY_STRICT_RISK_THRESHOLD || 0.4),
  ...
  MAX_ASSET_USES_PER_VIDEO: Number(process.env.MAX_ASSET_USES_PER_VIDEO || 5),
  ...
  BURN_CAPTIONS: toBool(process.env.BURN_CAPTIONS, false),
  ...
  ASSET_SEARCH_RESULTS_PER_QUERY: Number(process.env.ASSET_SEARCH_RESULTS_PER_QUERY || 20),
  ASSET_CANDIDATE_POOL_PER_SCENE: Number(process.env.ASSET_CANDIDATE_POOL_PER_SCENE || 30),
  ASSET_DOWNLOAD_TOP_PER_SCENE: Number(process.env.ASSET_DOWNLOAD_TOP_PER_SCENE || 12),
